...
**`outreach/variant_optimizer_job.py`** — the autonomous job, same architecture as the other outreach cron jobs (standalone script, direct DB access; Cowork/scheduled routines are confirmed blocked from writing to production, same as the discovery routine). Threshold-gated, not calendar-gated: intended to run hourly, but only acts on a stage once `MIN_SAMPLE_SIZE` (30, Section 5b's standard) new outcome-mature touches have accumulated since that stage's `VariantOptimizerState` marker — naturally near-silent at current volume, naturally responsive once volume scales, no manual retuning. `MIN_SAMPLE_SIZE` is also the "eyes before judgment" gate — a specific variant's own performance/deliverability is never judged until it has 30 real sends of its own, independent of whether the stage-level threshold above has been crossed.

Per triggered stage: computes per-variant open/click/paid rates (click is the primary metric for pre-click stages initial/A/B; paid for post-click C/D, since those cohorts have already clicked by definition); runs `outreach/stats_utils.py`'s two-proportion z-test for each canary against the current best active variant; a confirmed winner's weight moves gradually toward the active pool (`WEIGHT_CONVERGENCE_FACTOR`, never an abrupt swap) and is promoted to `active` once it reaches parity; a confirmed loser is paused. Independent of significance testing, any variant whose own bounce/complaint rate breaches the same thresholds `outreach/ramp.py` uses channel-wide is auto-paused immediately (a deliverability safety action). Every significant result is written to `EvidenceFinding`.

**Candidate generation is a daily Claude Code routine, not a direct metered API call** (changed 2026-07-21, to avoid Anthropic API cost) — mirrors the nightly email-discovery routine's Drive-based handoff exactly, since routines are confirmed blocked from reaching the DB or pushing git, same as before: (1) when a stage needs a new candidate (no unresolved canary/request and under `MAX_VARIANTS_PER_STAGE`, 6 — a proliferation safety cap, not something the original build spec numbered), the hourly job reserves a `pending_generation` placeholder `EmailVariant` row and pushes every currently-pending request to `outreach/discovery_batches/variant_candidate_request.json` via the GitHub REST Contents API (`outreach/github_push.py`, `GITHUB_PUSH_TOKEN`); (2) the routine ("Groundwork daily email-variant candidate generation," `trig_01UNr3jg8jYU9jC8oEqeSeUR`, `0 5 * * *` UTC) reads that file plus the evidence library from its git checkout and writes one candidate per request to a Google Drive file (`groundwork-variant-candidates.json`, same shared folder the discovery routine uses); (3) the job's next run picks that up (`GOOGLE_DRIVE_API_KEY`/`GOOGLE_DRIVE_FOLDER_ID`, read-only, idempotent via `VariantCandidatePickupState`) and runs it through `outreach/content_safety.py`'s `run_content_safety_gates` (no built-claim pre-click, own-price accuracy, spam-trigger words) and `check_isolation` — admitting it as a canary at ~10% weight on success, or deleting the placeholder (freeing the slot for a fresh attempt) on failure.

**Section 3 is database-backed, not committed to the .md file in production.** `EvidenceFinding` rows ARE the current Section 3; `/admin/variants` renders them in the Finding/Sample size/Isolated variable/Rationale shape the file's Section 3 describes. `docs/cold-email-evidence-library.md`'s copy of Section 3 stays a static stub in git. (Unlike the earlier assumption here, a Railway Cron container CAN push to git via the GitHub Contents API with a token — proven by the request-file export above — so this is now a choice to keep Section 3 as one clean queryable table, not a hard platform constraint like the routine's git-push block is.)
...
**Genuinely tested** (`test_variant_system.py`, plain script against a throwaway SQLite DB, no pytest in this project yet): all 5 shipped baseline templates pass every content-safety gate; the isolation heuristic catches a multi-axis change and passes a genuinely single-axis one; the z-test distinguishes a real rate difference from noise; seeding is idempotent; weighted selection tracks weight statistically; the full `_process_stage` pipeline was run against synthetic data for all four real branches — threshold-not-met no-op, confirmed-winner promotion, confirmed-loser pause, and deliverability auto-pause — plus the full Drive-based request/pickup pipeline with `push_file_to_github` and the Drive download both mocked (request-queueing, a good result admitted, a bad-isolation result rejected and its placeholder deleted, and pickup idempotency on a repeat run). Also ran the real job once against production (safe no-op, 0 samples, since no touch had a `variant_id` yet) to prove the whole pipeline executes cleanly end to end. **Not tested**: the routine's actual generation quality (no real routine run has happened yet — first fires 2026-07-22 05:00 UTC) and the job's behavior under real production send volume over time.
...
## 20. Facebook DM Outreach (manual channel, added 2026-07-24)

A third outreach channel alongside email and SMS, for the `no_website` segment specifically — but deliberately **not automated**, unlike everything else in this pipeline.

**Capture**: the nightly email-discovery routine already runs `site:facebook.com "[business name]" "[town]"` searches as one of its lookup strategies. `Prospect.facebook_page_url` (new column) now keeps whatever Facebook Page URL that search turns up, **regardless of whether the same search ultimately found a usable email or not** — previously this was discarded the moment email discovery resolved either way. `outreach/import_discovery_results.py`'s expected JSON schema gained an optional `facebook_page_url` key per entry, applied unconditionally before the email-found/not-found branching.

**Why manual, not automated**: sending a Messenger DM programmatically (Messenger Platform API without the recipient having messaged first, or worse, browser automation driving a logged-in Facebook session) breaks Facebook's Terms of Service and risks the account being restricted — a risk not worth taking for one outreach channel. The tool's job is only to make the human-driven loop (find → copy → paste → send → log) fast, never to remove the human from actually clicking send.

**`/admin/facebook-outreach`** (session-gated, linked from `/admin/pipeline`): lists every `no_website` prospect with a captured `facebook_page_url` and no `facebook_dm_sent_at` yet, highest-scored first. Each row: a link that opens the Facebook Page in a new tab, a pre-filled message (`outreach/templates.py:render_facebook_dm`, same voice as `INITIAL_SMS` adapted to a first-person DM opener) with the prospect's real magic link merged in (`ensure_link_identity` generates one on the fly if this prospect has never been sent anything via another channel yet), a copy-to-clipboard button, and a "Mark as sent" button.

**Logging**: marking a DM as sent writes a real `OutreachTouch` row (`channel="facebook"`, `stage="initial"`) — the same table email/SMS sends write to — plus sets `Prospect.facebook_dm_sent_at`, bumps `touch_count`/`last_touch_at`, and advances `funnel_stage`/`funnel_substage` to `"sent"` only if the prospect hadn't been sent anything via any channel yet (never regresses a prospect already further along). This makes Facebook DM performance directly comparable against email/SMS in the existing funnel views, without any separate reporting path.

**Not yet built**: a follow-up drip for this channel (currently `initial` only, matching the "no automation" constraint — a follow-up DM still requires the same manual send), and no attempt to detect a DM "opened"/"seen" state (Messenger doesn't expose that without the Platform API this channel deliberately avoids).
