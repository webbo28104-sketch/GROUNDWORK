...
- Do NOT include `og:url`, a JSON-LD `url` field, or a canonical `<link>` tag — the live URL isn't assigned until after generation, so there is no real value to put there; inventing one would be a fabricated fact. Leave these out entirely rather than guessing.
- JSON-LD structured data — one `<script type="application/ld+json">` block in `<head>` using schema.org `LocalBusiness` (or a more specific applicable subtype if obviously correct, e.g. `RoofingContractor`, `Electrician`, `Plumber` — only if clearly matching the trade, otherwise plain `LocalBusiness`):
...
- section: the page section (hero, nav, about, services, portfolio, accreditations, testimonials, faq, contact, footer)
