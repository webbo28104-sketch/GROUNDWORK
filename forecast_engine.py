...
class CashFlowEngine:
    ...
    def add_invoice(self, due_date: datetime, amount: float, reference: str, contact: str,
                     ...
                     retention_pct: float = 0.0, retention_release_days: int = 180) -> None:
        ...
        if retention_pct > 0:
            ...
            upfront = net - held
            self.events.append(CashEvent(due_date, upfront, "incoming", reference, contact, probability))
            self.events.append(CashEvent(
                due_date + timedelta(days=retention_release_days), held, "incoming",
                f"{reference} (retention)", contact, probability,
            ))
        else:
            self.events.append(CashEvent(due_date, net, "incoming", reference, contact, probability))
    ...
    def add_staged_invoice(self, stages: list, reference: str, contact: str, probability: float = 1.0,
                            ...
                            retention_release_days: int = 180) -> None:
    ...
    def run_scenario(self, scenario_changes: dict) -> dict:
        ...
        original_events = self.events
        test_events = [CashEvent(e.date, e.amount, e.event_type, e.reference, e.contact, e.probability) for e in original_events]
