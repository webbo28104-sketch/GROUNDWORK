...
- **`send-job-cron` runs HOURLY, not once a day** (changed 2026-07-21) — its Railway Cron schedule needs to be `0 8-19 * * *` (UTC), not a single daily firing. Email (initial sends + follow-ups) only actually sends inside that 08:00-19:00 UTC window and is capped per-hour (`outreach/ramp.py`'s `EMAIL_HOURLY_RAMP_TABLE`, week-1 floor **42/hour** — raised from 5 the same day, an explicit override to clear the awaiting_approval backlog while the circuit breaker was tripped; see that file's comment), ramping the same conservative way the old per-day table did, just denominated per-hour — the deliberate point is to spread sends evenly across the day so real click/open data can tell us which hours perform best, rather than firing one lump batch at a fixed time. Sourcing-cron (free to run as often as needed) is meant to be the actual volume limiter, not an artificially low per-hour cap. SMS is untouched — still one daily budget via `get_remaining_ramp_today`, no window. `run_daily_send()` (the function name predates this change) is still the entry point `send_job.py`'s `__main__` calls; being invoked hourly instead of daily is safe since every underlying check (`get_remaining_ramp_this_hour`, follow-up due-dates) is naturally idempotent per invocation.
