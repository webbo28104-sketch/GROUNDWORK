...
- `outreach/pipeline.py` (sourcing), `outreach/email_discovery_job.py` (Tier 1 email scraping, free/code-only), `outreach/send_job.py` (sends), `outreach/domain_billing.py`, `apply_pending_edits_job.py` — each is a standalone Python script pointed at by its own Railway Cron service (`sourcing-cron`, `email-discovery-cron`, `send-job-cron`, `domain-billing-cron`, `pending-edits-apply-cron`), nothing in-process schedules them.
