...
- `outreach/pipeline.py` (sourcing), `outreach/email_discovery_job.py` (Tier 1 email scraping, free/code-only), `outreach/send_job.py` (sends), `outreach/domain_billing.py`, `apply_pending_edits_job.py`, `outreach/variant_optimizer_job.py` (email-variant testing, Section 19) — each is a standalone Python script pointed at by its own Railway Cron service (`sourcing-cron`, `email-discovery-cron`, `send-job-cron`, `domain-billing-cron`, `pending-edits-apply-cron`, `variant-optimizer-cron`), nothing in-process schedules them.
- **`send-job-cron` runs HOURLY, not once a day** (changed 2026-07-21) — its Railway Cron schedule is `0 3-22 * * *` (UTC), not a single daily firing. Widened from `0 8-19 * * *` the same day (still 2026-07-21) after two real magic-link clicks landed ~9-10pm BST — the point is to gather real peak/trough click data across a much wider span of the day (04:00-23:00 BST, 20 hourly slots) rather than assume a 9-5 business-hours pattern; both the Railway cron field AND `outreach/ramp.py`'s `EMAIL_SEND_WINDOW_START_HOUR`/`END_HOUR_EXCLUSIVE` were updated together — they have to agree, since the code-level check is a second guard, not the only enforcement. Email (initial sends + follow-ups) only actually sends inside that window and is capped per-hour (`outreach/ramp.py`'s `EMAIL_HOURLY_RAMP_TABLE`, week-1 floor **42/hour** — raised from 5 earlier the same day, an explicit override to clear the awaiting_approval backlog while the circuit breaker was tripped; see that file's comment). Widening the window to 20 hours at the same 42/hour floor raises the theoretical daily ceiling from ~504 to ~840 — not rescaled down, since the ask was specifically about timing coverage, not volume; flag if you'd rather cap total daily volume by lowering the per-hour floor instead. Sourcing-cron (free to run as often as needed) is meant to be the actual volume limiter, not an artificially low per-hour cap. SMS is untouched — still one daily budget via `get_remaining_ramp_today`, no window. `run_daily_send()` (the function name predates this change) is still the entry point `send_job.py`'s `__main__` calls; being invoked hourly instead of daily is safe since every underlying check (`get_remaining_ramp_this_hour`, follow-up due-dates) is naturally idempotent per invocation.
