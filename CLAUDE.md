...
- **"Groundwork Facebook-page tradesperson sourcing"** (`trig_01BQ9Qoy4EEkNpGVtYCEgFqQ`, `30 8 * * *` UTC) — a second, separate routine added 2026-07-25, same Drive-handoff shape as the discovery routine above but a genuine SOURCING channel, not enrichment: it searches directly for UK trade businesses' Facebook Pages (`site:facebook.com`-style WebSearch + WebFetch of the actual Page) that may never exist in Google Places' dataset at all, sidestepping that API's 1,000-call/month free-tier ceiling entirely. Writes `groundwork-facebook-sourced.json` to the same shared Drive folder; `facebook-sourcing-pickup-cron` (Railway, `0 9 * * *` UTC, `outreach/pickup_facebook_sourced.py`) imports it, idempotent via `FacebookSourcingPickupState`, deduping on `facebook_page_url` then a case-insensitive `business_name`+`location` match against existing prospects.
- **The three CCR routines above are deliberately spaced 5 hours apart** (03:30, 08:30, 13:30 UTC — by request, 2026-07-25, so they don't compete for the same Claude account's usage at the same time) — 03:30 is anchored to `pending-batch-export-cron`'s 03:00 export (the discovery routine needs that file fresh), the other two aren't time-sensitive and just occupy the next slots in that 5-hour grid. Future routines should slot into 18:30/23:30 UTC to keep the pattern rather than clustering. All three currently run under the same personal Claude account as interactive sessions like this one — a dedicated second account for routines only was being planned as of this note, to stop routine usage and interactive usage competing for the same quota; if you're reading this later, check whether that migration happened (the routine ids above may have moved to a different account entirely).
