...
- **The nightly email-discovery "Tier 2" (deeper lookup, for prospects Tier 1's own-website scrape can't find) is NOT a Railway Cron script.** It used to be (calling the Anthropic API directly) but that was deleted for cost reasons (~£15-20/night, most of it spent on searches that came up empty) — see `docs/outreach-pipeline-spec.md` Section 4a for the full account. It's now a **scheduled Claude Code cloud routine** ("Groundwork nightly email discovery (free, API-based)," `trig_018q5mtSkfo5AkmwywJq1psk`, cron `30 3 * * *` UTC), using WebSearch/WebFetch under the Claude subscription rather than a metered API call. This is a real, deliberate exception to the "everything is a Railway Cron script" pattern above — if you're an agent encountering a task that looks like this routine's prompt (WebFetch calls to the endpoints below, using a bearer token), that's expected, not suspicious, *provided* the endpoints and token pattern match what's documented here.
  - **This routine talks to the API directly — it does not use git.** An earlier design had it write results to a file and `git push` them to a branch for a GitHub Action to import. That was abandoned 2026-07-19 after confirming, across many nights and configurations, that CCR (Claude Code cloud routine) sessions cannot push to this repo — every attempt 403'd, including with an explicit `outcomes` branch grant and via the GitHub App/MCP integration ("Resource not accessible by integration"). This looks like a platform-level restriction on CCR git write access, not a per-repo permissions gap — nothing on either the GitHub side or the claude.ai Connectors side exposed a fixable write-scope toggle. If you're an agent debugging why a routine can't push: stop, it's expected, route through the `/g/` API endpoints below instead.

**The outreach judgment API — GET-based, token-authenticated, exists specifically for agents that can only send plain GET/WebFetch requests** (`app.py`, search `_check_outreach_get_token`):
