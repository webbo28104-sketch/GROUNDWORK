...
- `outreach/pipeline.py` (sourcing), `outreach/email_discovery_job.py` (Tier 1 email scraping, free/code-only), `outreach/send_job.py` (sends), `outreach/domain_billing.py`, `apply_pending_edits_job.py`, `outreach/variant_optimizer_job.py` (email-variant testing, Section 19), `outreach/send_daily_summary.py` (once-daily admin digest, added 2026-07-23) — each is a standalone Python script pointed at by its own Railway Cron service (`sourcing-cron`, `email-discovery-cron`, `send-job-cron`, `domain-billing-cron`, `pending-edits-apply-cron`, `variant-optimizer-cron`, and `daily-summary-cron` for the digest — **this last one still needs creating manually in the Railway dashboard**, scheduled for something like `15 23 * * *` UTC, safely after send-job-cron's window closes), nothing in-process schedules them.
...
- **`send-job-cron` runs every 15 MINUTES, not hourly** (changed 2026-07-23, by request) — its Railway Cron schedule needs to be `*/15 3-22 * * *` (UTC) — **this still needs setting manually in the Railway dashboard**, since this repo's `.railway/railway.ts` is a stub that doesn't model cron services and `railway config pull` isn't usable here (missing TS SDK dependency). The window itself is unchanged (03:00-22:00 UTC / 04:00-23:00 BST, 20 hours — `outreach/ramp.py`'s `EMAIL_SEND_WINDOW_START_HOUR`/`END_HOUR_EXCLUSIVE`); only the firing cadence and per-firing cap changed. Email now sends a fixed **5 per 15-minute slot** (`outreach/ramp.py`'s `EMAIL_PER_SLOT_CAP`/`EMAIL_SLOT_MINUTES`), replacing the old per-hour ramp-table figure (`EMAIL_HOURLY_RAMP_TABLE` still exists and still drives SMS's daily ramp, but email's per-firing cap is now this fixed number, not derived from the table). `get_remaining_ramp_this_hour()` (name predates this change) now buckets email by 15-minute slot internally, not by hour. SMS is untouched — still one daily budget via `get_remaining_ramp_today`, no window. `run_daily_send()` (the function name predates all of this) is still the entry point `send_job.py`'s `__main__` calls; every underlying check is idempotent per invocation regardless of firing frequency.
- **`variant-optimizer-cron` runs HOURLY** (`0 * * * *` UTC), pointed at `outreach/variant_optimizer_job.py` — but is threshold-gated, not calendar-gated (see `docs/outreach-pipeline-spec.md` Section 19), so most hourly runs at current volume are genuine no-ops. All DB-side work (stats, promotion/pause) is plain code, zero API cost. Needs `DATABASE_URL`, `GITHUB_PUSH_TOKEN` (pushes the candidate-request file via the GitHub Contents API), `GOOGLE_DRIVE_API_KEY`/`GOOGLE_DRIVE_FOLDER_ID` (reads the routine's candidate results) — no `ANTHROPIC_API_KEY` (removed 2026-07-21; candidate generation moved off a direct metered call to the daily routine below, by request, to avoid API cost). **Cannot itself be a Claude Code routine or run via Cowork** — both are confirmed blocked from writing to production, same reason every other send/scoring job in this list is a Railway Cron script and not a routine.
- **"Groundwork daily email-variant candidate generation"** (`trig_01UNr3jg8jYU9jC8oEqeSeUR`, `30 14 * * *` UTC) — a genuine Claude Code routine, same Drive-based handoff pattern as the nightly email-discovery routine (reads `outreach/discovery_batches/variant_candidate_request.json` + `docs/cold-email-evidence-library.md` from its git checkout, writes `groundwork-variant-candidates.json` to the shared Drive folder). Only generates copy — never touches the DB or the ramp. `variant-optimizer-cron` above both feeds this routine its work (via the GitHub-pushed request file) and picks up its output (via Drive).
- **The nightly email-discovery "Tier 2" (deeper lookup, for prospects Tier 1's own-website scrape can't find) is NOT a Railway Cron script.** It used to be (calling the Anthropic API directly) but that was deleted for cost reasons (~£15-20/night, most of it spent on searches that came up empty) — see `docs/outreach-pipeline-spec.md` Section 4a for the full account. It's now a **scheduled Claude Code cloud routine** ("Groundwork nightly email discovery (free, API-based)," `trig_018q5mtSkfo5AkmwywJq1psk`, cron `30 3 * * *` UTC), using WebSearch under the Claude subscription rather than a metered API call. This is a real, deliberate exception to the "everything is a Railway Cron script" pattern above.
  - **The routine can reach neither groundworkbuild.com nor git — both confirmed platform-level restrictions on its CCR sandbox, not config gaps.** Direct API calls (WebFetch to the `/g/` endpoints below) are blocked by the sandbox's own egress policy (403 at the CONNECT-tunnel stage — confirmed via its `/__agentproxy/status` diagnostic, unrelated to anything on the app side). `git push` 403s every time regardless of branch/`outcomes` config or GitHub App state ("Resource not accessible by integration") — confirmed via `git ls-remote` that no `claude/*` branch has ever actually landed on the remote from any CCR session. **What actually works**: the routine reads `outreach/discovery_batches/pending_batch.json` from its git checkout (`Read` only, no network — the file is refreshed nightly by `.github/workflows/export-pending-batch.yml`, which runs on GitHub Actions' normal unrestricted network), and writes results to a Google Drive file via its `Google_Drive` MCP connection — a different transport than the sandboxed WebFetch/git paths, and the only one that's been reliable (it worked even on a night WebFetch was broadly down for every other domain, confirmed via a bbc.co.uk control fetch). `.github/workflows/pickup-discovery-results.yml` (06:00 UTC, unattended) reads that Drive file with a plain API key and imports it via `outreach/pickup_drive_results.py` — idempotent via `DiscoveryImportState`, so it's safe to run daily regardless of whether the routine produced anything new that night. If you're an agent debugging why a routine can't push or can't reach the app: stop, it's expected — don't try to "fix" it by routing around it, the Drive path is the sanctioned one.

- **"Groundwork Facebook-page tradesperson sourcing"** (`trig_01BQ9Qoy4EEkNpGVtYCEgFqQ`, `30 9 * * *` UTC) — a second, separate routine added 2026-07-25, same Drive-handoff shape as the discovery routine above but a genuine SOURCING channel, not enrichment: it searches directly for UK trade businesses' Facebook Pages (`site:facebook.com`-style WebSearch + WebFetch of the actual Page) that may never exist in Google Places' dataset at all, sidestepping that API's 1,000-call/month free-tier ceiling entirely. Writes `groundwork-facebook-sourced.json` to the same shared Drive folder; `facebook-sourcing-pickup-cron` (Railway, `0 10 * * *` UTC, `outreach/pickup_facebook_sourced.py`) imports it, idempotent via `FacebookSourcingPickupState`, deduping on `facebook_page_url` then a case-insensitive `business_name`+`location` match against existing prospects.
- **The three CCR routines above are deliberately spaced 5 hours apart** (04:30, 09:30, 14:30 UTC — by request, 2026-07-25, so they don't compete for the same Claude account's usage at the same time). 04:30 is the email-discovery routine's PRE-EXISTING schedule (anchored to `pending-batch-export-cron`'s 03:00 export — don't move it earlier than ~03:30 or it reads stale data); the other two just occupy the next slots in that 5-hour grid from there. Future routines should slot into 19:30/00:30 UTC to keep the pattern rather than clustering. All three currently run under the same personal Claude account as interactive sessions like this one — a dedicated second account for routines only was being set up as of this note, to stop routine usage and interactive usage competing for the same quota; if you're reading this later, check whether that migration happened (the routine ids above may have moved to a different account entirely, or these three may be disabled here and re-created there).
